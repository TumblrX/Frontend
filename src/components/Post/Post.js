  ...
  const postContent = content.map((contentBlock) => {
    if(contentBlock.type === 'text'){
      ...
      if(!subtype){
        ...
        if(subtype==='heading1'){
        ...
        else if(subtype==='heading2'){
          return <h2>{text}</h2>
        }
        else if(subtype==='quirky'){
          ...
          return(
            <p className={classes.chat} dangerouslySetInnerHTML={{__html: text}}></p>
          )
        }
      ...
      return(
        <figure className={classes.image}>
            <img src={contentBlock.url} alt={contentBlock.altText} width={contentBlock.width} height={contentBlock.height}/>
      ...
      if(contentBlock.provider==='soundcloud'){
        audio=<iframe width="100%" height="166" src={contentBlock.url} frameborder="no"></iframe>
      }else {
      ...
      if(contentBlock.provider==='youtube'){
        video=<iframe width="100%" height="315" src={`//www.youtube.com/embed/${getYoutubeVideoId( contentBlock.url)}`} frameborder="0" allowfullscreen></iframe>;
      }else {
        //Here provider is tumblrx
      }
      return(
